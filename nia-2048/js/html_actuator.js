...
HTMLActuator.prototype.message = function (ended) {
  ...
  var message = window.game.won ? result_msg + "" : "ee们加油呀~";
  if(!window.game.won) {
    if(window.game.maxTile >= 1024) message = "离心头肉只差一步~";
    else if(window.game.maxTile >= 512) message = "ee们，肉宝可爱吗？";
    else if(window.game.maxTile >= 256) message = "肉宝很甜，ee继续加油~";
  }
  else if(window.game.maxTile >= 2048) {
    message = result_msg + "恭喜ee找到心头肉~";
  }
...
HTMLActuator.prototype.refreshRel = function (remainingTime) {
  if(remainingTime > 0){
    ...
    this.statusBar.textContent = "Move the bricks to Find Your Nia!";
